...
#define _GNU_SOURCE
#include "sysmon.h"
int store_info(header_t * head, char *fname)
{
	char str[128];
...
int store_result_by_wakeup(sum_block_t * sbk, char *fname)
{
	int i;
	header_t *head = sbk->h;
	int ret = 0;
	...
	char *f = (fname ? fname : DEFAULT_SYSMON_WAKEUP_RESULT);
	FILE *fd = fopen(f, "w");
	if (!fd) {
	...
	float wakeup = sbk->wakeup_delta * 1.0/(head->len - 1)/head->delay/head->cpunum;
	...
	i = 1;
	while (i < head->len){
		sysnode_t *s = &sbk->sa[i - 1];
		fprintf(fd, "        data.addRow([\"%d\", %3.1f]);\n", i, s->wakeup_delta * 1.0/head->delay/head->cpunum);
		i++;
	}
...
int store_result_by_cpu_freq(sum_block_t * sbk, char *fname)
{
	...
	/* Create result template */
	char *f = (fname ? fname : DEFAULT_SYSMON_CPUFREQ_RESULT);
	FILE *fd = fopen(f, "w");
	if (!fd) {
		fprintf(stderr, _("Fail open file"));
		return 1;
...
int store_result_by_rss(sum_block_t * sbk, char *fname)
{
	...
	char *f = (fname ? fname : DEFAULT_SYSMON_RSS_RESULT);
	FILE *fd = fopen(f, "w");
	if (!fd) {
		fprintf(stderr, _("Fail open file"));
		return 1;
	...
	while (item) {
		...
		if (spd->rss_avg < 2000)
			break;
		fprintf(fd, "        data.addColumn('number', '%s(%uK)');\n",
			spd->pname, spd->rss_avg);
...
int store_result_by_cpu_usage(sum_block_t * sbk, char *fname)
{
	...
	char *f = fname ? fname : DEFAULT_SYSMON_CPU_RESULT;
	FILE *fd = fopen(f, "w");
	if (!fd) {
		fprintf(stderr, _("Fail open file"));
		return 1;
